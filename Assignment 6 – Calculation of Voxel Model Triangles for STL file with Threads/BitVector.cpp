...
{
	bitArray = new MyArray<uint8_t>(size);
...
void BitVector::Fill()
{
	if (this->size % 8 == 0) {
		...
		bitArray->Set(numberOfArray - 1, 0xFF);
	}
}
...
bool BitVector::Get(int index)
{
	...
	return ((bitArray->Get(indexA)) & (1 << offset));
}
...
void BitVector::Set(int index, int value)
{
	...
	if (value) {
		...
		bitArray->Set(indexA, val | bitArray->Get(indexA));
	}
	else {
		//bitArray[indexA] = (~val) & bitArray[indexA];
		bitArray->Set(indexA, (~val) | bitArray->Get(indexA));
